...
Slide numbers refer to the rendered deck (`mosaic-tutorial.html`): 44 slides: 32 in
the main flow, then an appendix (33–44) after the closing slide. Timings live
...
| 0:03–0:15 | 3–8 | Part 1 · what MOSAIC is | Twelve minutes, no more. Slide 5 is the annotated screenshot — click through all six reveals, it is the fastest way to teach the task. Slide 6 (the collaboration loop) is the one slide that earns the tutorial its title; do not rush it. Slide 7 is the only architecture they need. |
| 0:15–0:42 | 9–18 | **Part 2 · install** | The hands-on block. Slide 10 is the whole install on one slide — leave it up while people catch up, then walk the same seven lines slowly. Announce a hard stop at 0:42. Walk the room. Slides 14–15 are the pygame-ce trap — say the fix out loud to the whole room the first time it comes up. Leave slides 17–18 (troubleshooting) up while you circulate. |
| 0:42–1:20 | 19–30 | **Part 3 · play, then tweak** | Two beats: play a mission (20–24), then turn the knobs (25–29). Slide 30 is the "try this now" card grid — that is where they should spend the last ten minutes of the block. |
| 1:20–1:30 | 31–32 | Next steps and Q&A | Appendix slides 34–44 are on hand for whatever they ask. |
...
the per-room semantics, and it is on slide 16.

**Someone will try `FullviewCamera` after slide 27 and get an `AttributeError`.**
...
`random.seed(...)` is needed too — that is slide 29, and it is a real bug on our
side, not user error.

**Someone will ask where the advisor lab went.** It is in the appendix
(slides 38–40) and in `labs/advisor.py`. Offer to walk it with them afterwards;
...
1. Slide 22 — what's on screen (slide 5 already annotates the same screenshot,
   and the controls slide covers the rest)
2. Slide 28 — knob 4, what mistakes cost
3. Slide 4 — why it exists (only if Part 1 is overrunning badly)

Never cut: slide 6 (the collaboration loop), the install slides, Part 2 in full,
the controls slide, playing a mission, or the "try this now" grid.

## Closing ask

End on slide 31 and make one concrete request: **open an issue when it breaks on
