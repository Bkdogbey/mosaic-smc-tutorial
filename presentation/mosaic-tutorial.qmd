...
[Why it exists → What it is → How it works]{.divider-path}
...
- **Meaningful decisions** — assistance can affect what the participant does.
- **Repeatable conditions** — comparable across participants.
- **Synchronized data** — actions, advice, outcomes and optional sensors, on one clock.
- **Reconfigurable components** — a new study without rebuilding the task.
...
::: {.hook}
A modular software testbed for studying human–AI collaboration in controlled, interactive tasks.
:::
...
The participant and what they see
:::
...
The advisor, when asked
:::

:::

::: {.arch-link}
⇅ actions · observations · advice
:::
...
One synchronized record of the whole session
:::
:::
...
::: {.band}
Actions, advice, outcomes and optional sensor data can be recorded throughout the interaction.
:::
...
Gymnasium and MiniGrid.
:::

::: {.card}
[Runs as]{.card-title}

Human-only, human–AI, or programmatic.
:::

::: {.card}
[Configured by]{.card-title}

Task, interface, advisor and experiment components, set independently.
:::
...
::: {.annotated}
![](assets/gui-screenshot.png){height="380"}

::: {.anno .fragment style="--x:34%; --y:11%; --w:6%; --h:8%;"}
Human operator — navigates the building
:::

::: {.anno .fragment .below style="--x:14%; --y:72%; --w:5%; --h:8%; --ly:8px; --lx:60px;"}
Victims and decoys — visual discrimination
:::

::: {.anno .fragment style="--x:26%; --y:59%; --w:42%; --h:22%; --lx:-60px;"}
Hazards and locked rooms — risk and planning
:::
...
::: {.punchline}
Rescue the real victims while managing hazards, uncertainty and [limited resources]{.accent}.
:::

::: {.note}
A study configuration can also introduce time pressure and declining victim health.
:::

::: {.punchline}
The task creates decisions about where to search, whom to rescue, and [when to rely on AI advice]{.accent}.
:::
...
The participant reads the current task.
:::
...
When asked, the AI responds using the available task observation.
:::
...
The participant follows, modifies, or rejects that advice.
:::
...
The environment changes and the cycle repeats.
:::

::::

::: {.band}
The complete interaction is recorded throughout the cycle.
:::

::: {.punchline}
The key question is whether — and how — AI advice [changes human decisions]{.accent}.
:::
...
::: {.card}
[Advice Quality]{.card-title}

How do correct, incorrect, or uncertain recommendations affect decisions?
:::

::: {.card}
[Human State]{.card-title}

How do time pressure and workload affect human–AI collaboration?
:::

::: {.card}
[Interface Design]{.card-title}

How do explanations, alerts, and feedback influence performance and trust?
:::

::::

::: {.note}
And: how do different AI models or advisory strategies change team behaviour?
:::

::: {.punchline}
In Parts Two and Three, we will install MOSAIC, launch a mission, and [modify its components]{.accent}.
:::
