...
## Start Your [Install]{.accent} Now

::: {.hook}
Do this first — it runs in the background while we talk.
:::
...
::: {.punchline}
Those last two lines are [not optional]{.accent} — Part 2 explains why.
:::

Three parts today: [what it is]{.accent} · [getting it running]{.accent} · [playing with it]{.accent}.
...
[What MOSAIC is]{.divider-subtitle}
...
::: {.hook}
A modular testbed for human–AI collaboration.
:::

A grid-based **search-and-rescue** simulation, built on [MiniGrid](https://github.com/Farama-Foundation/Minigrid), where a human operator, an AI advisor, or both together must rescue survivors from a burning building before time runs out.

Named for the art of building one coherent picture from many small tiles — which is both what the platform does and what the world literally looks like.

::: {.punchline}
Many pieces, [one picture]{.accent}.
:::

:::
::: {.column width="45%"}
![](assets/game-view.png)
:::
:::
...
::: {.arch-block}
[Human Operator]{.arch-title}

Keyboard in, eyes on the screen — seven actions, one key each
:::

::: {.arch-block}
[AI Advisor]{.arch-title}

Anything with a `query()` method — GPT, Gemini, a scripted rule, or the built-in stub
:::

:::

::: {.arch-link}
▼ key presses · advice requests  ·  ▲ what they see · what it says
:::

::: {.arch-row}
::: {.arch-block}
[Interface]{.arch-title}

Game view · mission status panel · chat panel · peripheral feedback at the screen edge
:::
:::
...
::: {.arch-row}
::: {.arch-block}
[Environment]{.arch-title}

The building, the survivors, the hazards, the scoring — every one of them a constructor argument
:::
:::
...
[Sensing & Logging]{.arch-title}

One record per frame, eye tracking on the same clock
:::

::: {.arch-block .secondary}
[Analytics]{.arch-title}

Frame-exact replay · areas of interest · workload surveys
:::
...
## Why It [Exists]{.accent}

:::: {.cards .cols-2 .fill .short}

::: {.card}
[Hard Enough]{.card-title}

The participant must genuinely *need* help. A task you can solve alone measures nothing about teaming.

::: {.card-meta}
Time budget · decaying survivors · lava · locked doors · decoys
:::
:::

::: {.card}
[Instrumented]{.card-title}

Every action, every glance, every piece of advice — timestamped and on one clock.

::: {.card-meta}
One record per frame, over Lab Streaming Layer
:::
:::
...
Another lab should be able to run your exact condition, not their approximation of it.

::: {.card-meta}
Seeded worlds · conditions in YAML · replay from the log
:::
:::

::: {.card}
[Reconfigurable]{.card-title}

Your manipulation should be a parameter, not a fork of the codebase.

::: {.card-meta}
World · view · scoring · logging · advisor — all injected
:::
:::
...
::: {.punchline}
Most testbeds get two of these. MOSAIC is our attempt at [all four]{.accent}.
:::
